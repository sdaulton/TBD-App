...
Then /^(?:|I )should see that "([^"]*)" has a price of "([^"]*)"$/ do |id, price|
  expect(page.find_by_id(id).find('td.price').text).to eq(price)
end

Then /^(?:|I )should see that "([^"]*)" has an image "([^"]*)"$/ do |id, img_name|
  fullpath = page.find_by_id(id).find('td.image').find('img')['src']
  expect(File.split(URI(fullpath).path)[-1]).to eq(img_name)
end

Then /^(?:|I )should see the image "([^"]*)"$/ do |img_name|
  fullpath = page.find('a.image').find('img')['src']
  expect(File.split(URI(fullpath).path)[-1]).to eq(img_name)
end

Then /^(?:|I )should see product (.*) in sorted order$/ do |field|
  values = []
  if field == "name"
    field = "productname"
  end
  page.all("td.#{field}").each { |x| values << x.text.gsub(/\$/, '') }
  if field == "price"
